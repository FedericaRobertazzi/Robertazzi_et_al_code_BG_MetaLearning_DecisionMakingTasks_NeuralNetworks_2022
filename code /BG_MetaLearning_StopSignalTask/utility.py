...
def get_Q_reset_hold_react(delay, a, b):
    Qreset=(a*delay+b)
    return Qreset

def reset_Q_delay(ACC_Q,delay,a,b):
    Qreset=get_Q_reset_hold_react(delay, a, b)
    for i in range(2):
        for j in range(2):
            if i == 0 and j == 0:
               ACC_Q[i][j].Q = Qreset+ np.random.uniform(0,0.3)
            elif i == 0 and j == 1:
               ACC_Q[i][j].Q = Qreset +np.random.uniform(0,0.3)
    return ACC_Q
...
def hold_react(MetaValue_Direction,MetaValue_Hold,beta,ACC_Q,MetaValue_Thr,delay,a,b):
    print('react done')
    if MetaValue_Hold[-1] <= MetaValue_Thr:
        beta.append(9+ np.random.uniform(0,0.1))
        ACC_Q = reset_Q_delay(ACC_Q,delay,a,b) 
    return ACC_Q, beta

def get_new_tau(Q,c,d):
    new_tau=(c*Q+d)
    return new_tau     